...
func (transaction TransactionElasticRepository) Search(reportID string, accountID string, initDate string, endDate string) (dto.SearchResponse, error) {
	layout := "2006-01-02"
	...
	response, err := transaction.Client.Search(
		transaction.Client.Search.WithIndex(os.Getenv("ELASTIC_INDEX")),
		transaction.Client.Search.WithBody(strings.NewReader(data)),
		transaction.Client.Search.WithTrackTotalHits(true),
	)
	if err != nil {
		return dto.SearchResponse{}, err
	}
