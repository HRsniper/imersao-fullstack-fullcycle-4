...
function withAuth<P, Q extends ParsedUrlQuery>(func: AuthServerSideProps<P, Q>): GetServerSideProps<P, Q> {
  return async (context) => {
    ...
    if (request?.subdomain && payload.subdomain !== request?.subdomain) {
      const destination = request.headers.host?.replace(request.subdomain, payload.subdomain) as string;
      return {
        redirect: {
          permanent: false,
