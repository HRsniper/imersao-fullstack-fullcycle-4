...
type Payload = KeycloakTokenParsed & KeycloakProfile & { subdomain: string };

type Token = { token: string; payload: Payload };

type Request = { headers: { cookie?: any } };

function validateAuth(req?: Request): Token | boolean {
...
function verifyToken(token: string, key: string): JwtPayload | false {
  try {
...
export { KEYCLOAK_PUBLIC_CONFIG, validateAuth, verifyToken, createAuthCookies };
export type { Payload, Token };
